...
Заказчик выбирается кнопкой **«Найти»**: открывается окно со поиском по ФИО
или телефону, выбранная карточка подставляет ФИО, телефон, автомобиль и госномер.
Заказчика можно и просто ввести строкой — тогда карточка создаётся при сохранении
заявки. Выпадающий список в поле не используется: он перезаполнялся при вводе
и сбрасывал место курсора, из-за чего курсор прыгал в начало строки.
...
Телефон показывается в едином виде **8 (999) 999-99-99**. Во время набора
поле остаётся обычным — цифры идут подряд, курсор двигается ровно, — а маска
ставится, когда поле покидают. В сохранённом виде номер всегда с маской:
любой формат (+7 912 345 67 89, 89123456789) приводится к одному виду.

Почему так: если подставлять скобки прямо во время ввода, приходится
переставлять место курсора, а Windows Forms при этом сама меняет текст —
курсор прыгает, и цифры попадают не на своё место.
...
## Перенос данных на новую версию

Меню **«Данные…» → «Выгрузить все данные…»** сохраняет **всё** в один файл `.zip`:
каталог номенклатуры с закупочными ценами и минимумами, наборы услуг, реквизиты
с темой и логотипом, заявки со статусами, заказчиков, склад и кассы с оплатами.

Меню **«Данные…» → «Загрузить данные…»** возвращает данные из этого файла. Перед
загрузкой программа показывает, что лежит в архиве и когда он сделан, и только
потом заменяет данные.

Что важно:

* **Перенос работает между режимами.** Архив, снятый при работе с файлами,
  загружается в базу данных, и наоборот.
* **Разделы, которых нет в архиве, не трогаются** — поэтому архив от старой
  версии программы загружается без потерь: чего в нём не было, то и не меняется.
* **Файл один**, его можно скопировать на другой компьютер или отправить почтой.
* **Чужой файл отклоняется** с понятным сообщением, а не портит данные.

Порядок перехода на новую версию: выгрузить данные в файл, обновить программу,
запустить её и загрузить данные из файла.
