...
Обе версии делают одно и то же: считают смету по прайс-листу, позволяют править цены,
...
прайс-лист встроены внутрь. Ни сервер, ни интернет не нужны — достаточно открыть файл
...
* вкладка **Прайс-лист**: правка позиций, разделов, артикулов, цен и единиц измерения
  из перечня (`шт.`, `услуга`, `литр`, `комплект`), добавление и удаление позиций,
  выгрузка и загрузка файла прайс-листа, возврат заводских цен;
...
* отметки, количества и правки прайс-листа сохраняются в браузере и переживают
...
* **Цена позиции правится прямо в смете** — колонка «Цена в смете». Цена в прайс-листе
...
* **Выгрузка и загрузка данных** — кнопка «Данные…» сохраняет прайс-лист, наборы услуг
...
PostgreSQL — тогда прайс-лист, наборы услуг и сметы становятся общими для нескольких
рабочих мест.
...
| Таблица | Что хранит |
|---|---|
| `smeta_prices` | прайс-лист: раздел, артикул, наименование, единица, цена, порядок |
...
Файл `*.smeta` — единый формат для обеих версий: в нём прайс-лист, наборы услуг, реквизиты
...
5. Кнопки сверху:
   ...
   * **Прайс-лист…** — открывает встроенный **редактор прайс-листа** (см. ниже);
...
## Редактор прайс-листа (внутри программы)

Кнопка **«Прайс-лист…»** открывает редактор цен — отдельное окно, в котором справочник
правится прямо в программе, без «Блокнота» и Excel.

![Редактор прайс-листа](tests/shot_editor.png)
...
| **Выгрузить в файл** | сохраняет прайс-лист в `.tsv` (Excel, резервная копия) |
| **Загрузить из файла** | заменяет прайс-лист данными из `.tsv` |
...
* Если файл прайс-листа защищён от записи или занят, программа предложит сохранить
...
* **«Выгрузить в файл»** сохраняет прайс-лист в `.tsv` — для Excel или резервной копии.
* **«Загрузить из файла»** заменяет текущий прайс-лист данными из `.tsv`
  (разделитель — знак табуляции, строки с `#` игнорируются). Удобно, когда прайс
...
* `tests\Harness2.cs` — 76 проверок редактора прайс-листа (карточек позиций): правка позиции и раздела
...
* Смету можно сформировать и без окна — по всему прайс-листу сразу:
